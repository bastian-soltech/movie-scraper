...
const data = await fetch(`https://api.ytbvideoly.com/api/resconsume/hotlist?res_type=${type}&page=${page}&count=10&lang=&from_channel=11&hot_order_type=plays_pv&source_domain=www.4khotvideo.com`,{
    headers: {
...
export async function getSeries(id) {
    const data = await fetch(`https://api.ytbvideoly.com/api/resconsume/tvshowlist?res_id=${id}&from_idx=1&to_idx=30&page=0&psize=0&by=name&order=0&web=1&channel=dubox&from_channel=11&clienttype=0&lang=PH&source_domain=www.4khotvideo.com`,{
    headers: {
        'User-Agent': 'Mozilla/5.0 (Windows NT 10.0; Win64; x64) AppleWebKit/537.36 (KHTML, like Gecko) Chrome/121.0.0.0 Safari/537.36',
        'Origin': 'https://www.4khotvideo.com',
        'Referer': 'https://www.4khotvideo.com/',
...
const json = await data.json()
const seriesDetail = await json.data
